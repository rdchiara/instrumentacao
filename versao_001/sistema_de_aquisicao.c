...
#define PPR             600.0f      // Pulsos por revolução do encoder
#define QUADRATURE      4.0f       // Contagem em quadratura completa
...
void beep(int duration_ms) {
    ...
    vTaskDelay(pdMS_TO_TICKS(duration_ms)); // Espera a duração do bip
...
// // ===================================================================================
// //              VERSÃO FINAL OTIMIZADA PARA ENCODER DE ALTA RESOLUÇÃO
// // ===================================================================================
// void encoder_task(void *pvParameters) {
//     ESP_LOGI(TAG, "Tarefa do Encoder (Alta Resolução) iniciada.");
...
//         // Roda a cada 10ms para garantir que nenhum pulso seja perdido
//         int current_A = gpio_get_level(ENCODER_A_PIN);
//         int current_B = gpio_get_level(ENCODER_B_PIN);

//         if (current_A != last_A || current_B != last_B) {
//             if (last_A == 0 && current_A == 1) { // Borda de subida em A
//                 g_pulse_count += (current_B == 0) ? 1 : -1;
//             } else if (last_A == 1 && current_A == 0) { // Borda de descida em A
...
//         // --- 2. CÁLCULO DE VELOCIDADE INSTANTÂNEA ---
//         // Com 600 PPR, podemos calcular a velocidade a cada ciclo de 10ms.
...
//             // g_filtered_speed_hz = current_hz; // Atualiza diretamente sem filtro
//         }

//         // --- 3. APLICA O FILTRO DE MÉDIA MÓVEL ---
//         speed_samples[sample_index] = current_hz;
//         sample_index = (sample_index + 1) % FILTER_DEPTH;
        
//         // --- 4. ATUALIZA A VARIÁVEL GLOBAL COM O VALOR FILTRADO ---
...
//         // --- 5. FREQUÊNCIA DA TAREFA ---
//         vTaskDelay(pdMS_TO_TICKS(10)); // Continua rodando a 100Hz para máxima precisão
//     }
// }
...
// ===================================================================================
//              VERSÃO FINAL COM HARDWARE PCNT PARA ALTA PRECISÃO
// ===================================================================================

// Handle global para a unidade do PCNT
pcnt_unit_handle_t pcnt_unit = NULL;

// ===================================================================================
//              VERSÃO CORRIGIDA DA INICIALIZAÇÃO DO PCNT
// ===================================================================================

void initialize_encoder_pcnt(void) {
    ESP_LOGI(TAG, "Inicializando encoder com hardware PCNT...");
    
    // Configuração da unidade do PCNT
    ...
    // Define como os canais reagem às bordas para decodificar em quadratura
    ...
    // Habilita a unidade do PCNT ANTES de dar o start
    ESP_ERROR_CHECK(pcnt_unit_enable(pcnt_unit));

    // Agora sim, inicia a contagem
    ESP_ERROR_CHECK(pcnt_unit_start(pcnt_unit));
    ESP_ERROR_CHECK(pcnt_unit_clear_count(pcnt_unit)); // Zera a contagem inicial
    ESP_LOGI(TAG, "PCNT iniciado.");
}

// ===================================================================================
//              VERSÃO FINAL COM RESET DO CONTADOR DE HARDWARE (SEM FILTRO)
// ===================================================================================
void encoder_task(void *pvParameters) {
    ESP_LOGI(TAG, "Tarefa do Encoder (PCNT com Reset) iniciada.");

    // Variáveis para o cálculo de velocidade
    int64_t last_time_ms = esp_timer_get_time() / 1000;
    
    // Zera a contagem do hardware no início para garantir um estado limpo
    if (pcnt_unit) {
        ESP_ERROR_CHECK(pcnt_unit_clear_count(pcnt_unit));
    }

    while (1) {
        // Roda a cada 20ms para sincronizar com a tarefa de aquisição (50Hz)
        vTaskDelay(pdMS_TO_TICKS(10));

        int64_t now_ms = esp_timer_get_time() / 1000;
        int64_t dt_ms = now_ms - last_time_ms;
        
        // Pega o número de pulsos contados PELO HARDWARE desde a última leitura
        int pulses_diff = 0;
        pcnt_unit_get_count(pcnt_unit, &pulses_diff);
        
        // >>>>> A MÁGICA FINAL ESTÁ AQUI: Zera o contador de hardware <<<<<
        // A próxima contagem começará do zero.
        ...
        float current_hz = 0.0f;
        if (dt_ms > 0) {
            // PPR é 600. O hardware PCNT já conta as 4 bordas (quadratura).
            float revolutions = (float)pulses_diff / (PPR * 4.0f);
            current_hz = fabsf((revolutions * 1000.0f) / dt_ms);
        }
        
        // Atualiza a variável global com a leitura instantânea e precisa
        g_filtered_speed_hz = current_hz;
    }
}
...
void app_main(void) {
    ...
    gpio_config_t io_conf_leds = {
        .pin_bit_mask = (1ULL << LED_GREEN_PIN) | (1ULL << LED_RED_PIN) | (1ULL << LED_YELLOW_PIN) | (1ULL << LED_WIFI_PIN) | (1ULL << BUZZER_PIN), 
